...
tasktimes_file = 'rpw\\file\\tasktime.txt'
taskNames_file = 'rpw\\file\\tasknames.txt'
edges_nodes_file = 'rpw\\file\\edges_nodes.txt'
workdays_worktime_annualDemand_file = 'rpw\\file\\demand_worktime.txt'
timeMultiplier = {'hrs':1, 'min':60, 'sec':3600}

def getDataFromFiles(workingDir):
    try:
        # Begin by reading in the data from text files
        with open(os.path.join(workingDir, tasktimes_file.replace("/", os.path.sep))) as f:
            taskTimes = f.read().splitlines()
            print(f"Task Times File Path: {os.path.join(workingDir, tasktimes_file.replace('/', os.path.sep))}")
            print(f"Task Times Content: {taskTimes}")

        with open(os.path.join(workingDir, taskNames_file.replace("/", os.path.sep))) as f:
            taskNames = f.read().splitlines()
            print(f"Task Names File Path: {os.path.join(workingDir, taskNames_file.replace('/', os.path.sep))}")
            print(f"Task Names Content: {taskNames}")

        with open(os.path.join(workingDir, edges_nodes_file.replace("/", os.path.sep))) as f:
            edges_nodes = f.read().splitlines()
            print(f"Edges Nodes File Path: {os.path.join(workingDir, edges_nodes_file.replace('/', os.path.sep))}")
            print(f"Edges Nodes Content: {edges_nodes}")

        with open(os.path.join(workingDir, workdays_worktime_annualDemand_file.replace("/", os.path.sep))) as f:
            worktimeDemand = f.read().splitlines()
            print(f"Worktime Demand File Path: {os.path.join(workingDir, workdays_worktime_annualDemand_file.replace('/', os.path.sep))}")
            print(f"Worktime Demand Content: {worktimeDemand}")
...
def plotGraph(G, ofname, title):
    ...
    plt.title(title)
    nx.draw(G, pos, with_labels=True, node_size=700, node_color="skyblue", font_size=8, font_color="black", font_weight="bold", arrowsize=20, linewidths=0, width=2)
    plt.savefig(ofname)
    plt.show()
# End of plotGraph()
